...
export function ProductsPreview() {
  ...
  return (
    <section ref={ref} className="relative py-28 overflow-hidden" style={{ background: "linear-gradient(180deg, #030e22 0%, #020818 100%)" }}>
      ...
      <div className="container mx-auto px-6 lg:px-16 relative z-10">
        ...
        <div className={`text-center mb-16 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="text-cyan-400 text-sm font-semibold uppercase tracking-[0.2em]">What We Offer</span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {products.map((p, i) => (
            <div
              ...
              className={`glass-card rounded-2xl p-6 flex flex-col gap-4 cursor-default group transition-all duration-500 hover:-translate-y-2 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
              style={{
                transitionDelay: inView ? `${i * 0.12}s` : "0s",
                borderColor: active === i ? `${p.color}44` : undefined,
                boxShadow: active === i ? `0 0 40px ${p.color}18` : undefined,
              }}
            >
              {/* Icon */}
              <div
              ...
              {/* Arrow link */}
              <div
                className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider mt-2 transition-all duration-300 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0"
                style={{ color: p.color }}
              >
                Learn More
        ...
        <div className={`text-center mt-16 transition-all duration-700 delay-500 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          <button className="group inline-flex items-center gap-3 px-8 py-4 rounded-full border border-cyan-400/30 text-cyan-400 font-semibold text-sm tracking-wide hover:bg-cyan-400/10 hover:border-cyan-400/60 transition-all duration-300">
            View All Solutions
